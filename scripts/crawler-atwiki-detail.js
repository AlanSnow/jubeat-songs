 ...
 * 用于后续提取封面图、BPM、难度变化等详细信息
 *
 * 用法:
 *   node scripts/crawler-atwiki-detail.js [options]
 *
 * 选项:
 *   --all              拉取所有未缓存的歌曲
 *   --id=123           拉取指定 ID 的歌曲
 *   --range=100-200    拉取 ID 范围内的歌曲
 *   --concurrent=5     并发数（默认：5）
 *   --delay=1000       请求间隔毫秒（默认：1000）
 */
...
async function downloadPage(browser, id, retryCount = 0) {
  ...
  try {
    ...
    // 拦截并阻止 CSS/JS/图片等资源加载，只保留 HTML
    await page.route('**/*', (route, request) => {
      const resourceType = request.resourceType();
      const url = request.url();

      // 只保留 HTML 文档和 XHR/Fetch 请求，阻止其他所有资源
      if (resourceType === 'document') {
        route.continue();
      } else if (['stylesheet', 'script', 'image', 'font', 'media', 'other'].includes(resourceType)) {
        // 阻止 CSS、JS、图片、字体、媒体等
        route.abort('aborted');
      } else if (resourceType === 'xhr' || resourceType === 'fetch') {
        // 可选：阻止 API 请求（如果需要）
        route.abort('aborted');
        // 这里选择继续，以防页面需要某些 API 来渲染
        // route.continue();
      } else {
        route.abort('aborted');
      }
    ...
    const response = await page.goto(url, {
      waitUntil: 'domcontentloaded', // 改为 domcontentloaded，不等待网络idle
    ...
    // 获取页面 HTML（不需要等待 domcontentloaded，因为 goto 已经等待了）
...
async function batchDownload(browser, ids, concurrent = CONFIG.concurrent) {
  ...
  async function worker() {
    while (index < ids.length) {
      ...
      const result = await downloadPage(browser, id);
...
async function main() {
  ...
  try {
    ...
    if (options.ids.length > 0) {
      ...
      // 默认：显示帮助
      console.log(`
用法: node scripts/crawler-atwiki-detail.js [options]
...
示例:
  node scripts/crawler-atwiki-detail.js --all
  node scripts/crawler-atwiki-detail.js --id=103
  node scripts/crawler-atwiki-detail.js --range=100-200 --concurrent=3
      `);
      return;
    }
