...
// 版本按时间倒序排列（最新的在前）
const VERSIONS = ['全部', '音乐魔方', 'Beyond the Ave.', 'Ave.', 'festo', 'clan', 'Qubell', 'prop', 'saucer fulfill', 'saucer', 'copious', 'knit', 'ripples', 'jubeat'];

// 等级滑条刻度：1-8 (整数), 9.0-9.9, 10.0-10.9
...
function formatLevel(level: number): string {
  ...
  return `Lv.${level.toFixed(1)}`;
}

// 获取等级的索引
function getLevelIndex(level: number): number {
  return LEVEL_TICKS.findIndex(tick => Math.abs(tick - level) < 0.001);
}

// 查找最接近的等级刻度
function findClosestLevel(value: number): number {
  return LEVEL_TICKS.reduce((prev, curr) =>
...
function getActiveFilterCount(filters: FilterState): number {
  ...
  // 等级范围：默认 1-10.9，如果不是默认值则算作活跃过滤
  if (filters.minLevel !== 1 || filters.maxLevel !== 10.9) count++;
        ...
        <div className={`rounded-2xl border shadow-sm backdrop-blur-sm ${
          ...
          {activeCount > 0 && (
            <div className={`px-3 pb-2 flex flex-wrap gap-1.5 ${filters.search ? '' : 'pt-2'}`}>
              ...
              {(filters.minLevel !== 1 || filters.maxLevel !== 10.9) && (
                <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs ${
                  isDarkMode ? 'bg-emerald-500/20 text-emerald-300' : 'bg-emerald-100 text-emerald-700'
                }`}>
                  {formatLevel(filters.minLevel ?? 1)}-{formatLevel(filters.maxLevel ?? 10.9)}
                </span>
              )}
...
function FilterContent({ tempFilters, updateTempFilter, isDarkMode, selectClass, labelClass }: FilterContentProps) {
  return (
    <div className="space-y-5">
      ...
      {/* 等级范围 */}
      <LevelRangeFilter
        minLevel={tempFilters.minLevel ?? 1}
        maxLevel={tempFilters.maxLevel ?? 10.9}
        onChange={(min, max) => {
          updateTempFilter('minLevel', min);
          updateTempFilter('maxLevel', max);
        }}
        isDarkMode={isDarkMode}
        labelClass={labelClass}
      />
...
function LevelRangeFilter({ minLevel, maxLevel, onChange, isDarkMode, labelClass }: LevelRangeFilterProps) {
  const minIndex = getLevelIndex(minLevel);
  const maxIndex = getLevelIndex(maxLevel);

  const handleMinChange = (newIndex: number) => {
    const newMin = LEVEL_TICKS[newIndex];
    if (newMin <= maxLevel) {
      onChange(newMin, maxLevel);
    }
  };

  const handleMaxChange = (newIndex: number) => {
    const newMax = LEVEL_TICKS[newIndex];
    if (newMax >= minLevel) {
      onChange(minLevel, newMax);
    }
  };

  const decreaseMin = () => {
    if (minIndex > 0) {
      handleMinChange(minIndex - 1);
    }
  };
  ...
  const decreaseMax = () => {
    if (maxIndex > minIndex) {
      handleMaxChange(maxIndex - 1);
    }
  };

  const increaseMax = () => {
    if (maxIndex < LEVEL_TICKS.length - 1) {
      handleMaxChange(maxIndex + 1);
    }
  };

  // 计算滑条位置百分比
  const minPercent = (minIndex / (LEVEL_TICKS.length - 1)) * 100;
  const maxPercent = (maxIndex / (LEVEL_TICKS.length - 1)) * 100;

  const sliderBgClass = isDarkMode ? 'bg-gray-700' : 'bg-gray-200';
  const rangeClass = 'bg-pink-500';
  const thumbClass = 'bg-white border-2 border-pink-500 shadow-md';
  ...
  return (
    <div className="space-y-3">
      ...
      <div className={`text-sm font-medium ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>
        {formatLevel(minLevel)} - {formatLevel(maxLevel)}
      </div>
      ...
      <div className="space-y-2">
          ...
          <div className="flex items-center gap-1">
            <button
              ...
              className={`w-6 h-6 rounded flex items-center justify-center text-xs font-bold transition-colors ${
                minIndex <= 0
                  ? isDarkMode ? 'bg-gray-700 text-gray-500' : 'bg-gray-100 text-gray-300'
                  : isDarkMode ? 'bg-gray-600 text-white hover:bg-gray-500' : 'bg-gray-200 text-gray-700 hover:bg-gray-300'
              }`}
            >
              -
            </button>
            <span className={`w-16 text-center font-medium ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>
              {formatLevel(minLevel)}
            </span>
            <button
              onClick={increaseMin}
              disabled={minIndex >= maxIndex}
              className={`w-6 h-6 rounded flex items-center justify-center text-xs font-bold transition-colors ${
                minIndex >= maxIndex
                  ? isDarkMode ? 'bg-gray-700 text-gray-500' : 'bg-gray-100 text-gray-300'
                  : isDarkMode ? 'bg-gray-600 text-white hover:bg-gray-500' : 'bg-gray-200 text-gray-700 hover:bg-gray-300'
        ...
        <input
          type="range"
          min={0}
          max={LEVEL_TICKS.length - 1}
          value={minIndex}
          onChange={(e) => handleMinChange(Number(e.target.value))}
          className="w-full h-2 rounded-lg appearance-none cursor-pointer"
          style={{
            background: `linear-gradient(to right, ${isDarkMode ? '#374151' : '#e5e7eb'} 0%, ${isDarkMode ? '#374151' : '#e5e7eb'} ${minPercent}%, #ec4899 ${minPercent}%, #ec4899 ${minPercent}%, ${isDarkMode ? '#374151' : '#e5e7eb'} ${minPercent}%, ${isDarkMode ? '#374151' : '#e5e7eb'} 100%)`
          }}
        />
      </div>
      ...
      <div className="space-y-2">
        <div className="flex items-center justify-between text-xs text-gray-500">
          ...
          <div className="flex items-center gap-1">
            ...
            <button
              onClick={increaseMax}
              disabled={maxIndex >= LEVEL_TICKS.length - 1}
              className={`w-6 h-6 rounded flex items-center justify-center text-xs font-bold transition-colors ${
                maxIndex >= LEVEL_TICKS.length - 1
                  ? isDarkMode ? 'bg-gray-700 text-gray-500' : 'bg-gray-100 text-gray-300'
                  : isDarkMode ? 'bg-gray-600 text-white hover:bg-gray-500' : 'bg-gray-200 text-gray-700 hover:bg-gray-300'
              }`}
            >
        ...
        <input
          ...
          className="w-full h-2 rounded-lg appearance-none cursor-pointer"
          style={{
            background: `linear-gradient(to right, ${isDarkMode ? '#374151' : '#e5e7eb'} 0%, ${isDarkMode ? '#374151' : '#e5e7eb'} ${maxPercent}%, #ec4899 ${maxPercent}%, #ec4899 ${maxPercent}%, ${isDarkMode ? '#374151' : '#e5e7eb'} ${maxPercent}%, ${isDarkMode ? '#374151' : '#e5e7eb'} 100%)`
          }}
        />
      </div>
    </div>
